...
import type { GoldenPathSeedV1 } from "./golden_path";
...
import { librariesSddlText } from "./libraries_spel";
import { patternsSddlText } from "./patterns_spel";
import { kitsSddlText } from "./kits_spel";
import { isPackLocked } from "./pack_governance";

export type GoldenPathSeedProposalResult =
  | {
      ok: true;
      proposal_id: string;
...
function buildSpecPackWithGoldenSeed(basePack: SpecPack, seed: GoldenPathSeedV1): { zip: Uint8Array; pack: SpecPack; warnings: string[] } {
  const warnings: string[] = [];

  const files: Record<string, Uint8Array> = {};
  for (const [path, f] of basePack.fileMap.entries()) files[path] = f.bytes;

  // Write deterministic SPEL modules
  const libsText = librariesSddlText({ library_ids: seed.recommended_library_ids });
  const patsText = patternsSddlText({ pattern_ids: seed.recommended_pattern_ids });
  const kitsText = kitsSddlText({ kit_ids: seed.recommended_kit_ids });

  files["spel/libraries.spel"] = new TextEncoder().encode(libsText);
  files["spel/patterns.spel"] = new TextEncoder().encode(patsText);
  files["spel/kits.spel"] = new TextEncoder().encode(kitsText);

  // Update manifest contents if needed
  try {
    ...
    if (parsed && typeof parsed === "object" && Array.isArray((parsed as any).contents)) {
      ...
      ensureManifestContains(contents, "spel/libraries.spel");
      ensureManifestContains(contents, "spel/patterns.spel");
      ensureManifestContains(contents, "spel/kits.spel");
...
export async function createGoldenPathSeedProposal(args: {
  project_id: string;
  state: ProjectState;
  seed: GoldenPathSeedV1;
}): Promise<GoldenPathSeedProposalResult> {
  ...
  if (isPackLocked(projectId)) {
    return {
      ...
      details: ["Golden Path seed proposals require an unlocked Spec Pack.", "Create a new project or unlock by switching projects."],
    };
  }
  ...
  // Build proposal Spec Pack
  const proposalBuilt = buildSpecPackWithGoldenSeed(basePack, args.seed);
  ...
  const patch = await buildPatchFromPacks({
    ...
    summary: `Golden Path seed (${args.seed.launch_path_id})`,
    stats: diff.stats,
  });

  const now = new Date().toISOString();
  const summary = `Golden Path seed: ${args.seed.launch_path_id}`;
